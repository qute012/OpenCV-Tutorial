...
def draw(args):
    ...
    (Pwidth, Pheight, Pmaxval, bitmap) = ppm_p6.read(infile)
    image = array.array('B', bitmap)
    for x in range(0, width):
        for y in range(0, height):
            nx = x + start_x
            ny = y + start_y
            image[RGB(width=Pwidth, x=nx, y=ny)['R']] = color[0]
            image[RGB(width=Pwidth, x=nx, y=ny)['G']] = color[1]
            image[RGB(width=Pwidth, x=nx, y=ny)['B']] = color[2]

    ppm_p6.write(Pwidth, Pheight, maxval, image, outfile)
